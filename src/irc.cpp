...
// Copyright (c) 2011-2012 Africoin Developers
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #AfricoinTEST0\r");
            Send(hSocket, "WHO #AfricoinTEST0\r");
        } else {
            // randomly join #Africoin00-#Africoin99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #Africoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #Africoin%02d\r", channel_number).c_str());
        }
